...
class TileProcessor:
    ...
    def __init__(self, div: int, overlap: int, input_shape: Tuple[int, int], 
        ...
        logger.debug(f"TileProcessor initialized: div={div}, overlap={overlap}, "
    ...
    def get_tile_coordinates(self, hindex: int, vindex: int) -> TileCoordinates:
        ...
        if self.div == 1:
            ...
            return TileCoordinates(
                xi1=0, xi2=self.xinp, yi1=0, yi2=self.yinp,
                xo1=0, xo2=self.xout, yo1=0, yo2=self.yout,
                xi1t=0, xi2t=self.xinp, yi1t=0, yi2t=self.yinp,
                xo1t=0, xo2t=self.xout, yo1t=0, yo2t=self.yout,
                xi1r=0, xi2r=self.xinp, yi1r=0, yi2r=self.yinp,
                xo1r=0, xo2r=self.xout, yo1r=0, yo2r=self.yout
            )
    ...
    def _calculate_base_coords(self, hindex: int, vindex: int) -> TileCoordinates:
        ...
        return TileCoordinates(
            xi1=xi1, xi2=xi2, yi1=yi1, yi2=yi2,
            xo1=xo1, xo2=xo2, yo1=yo1, yo2=yo2,
            xi1t=0, xi2t=0, yi1t=0, yi2t=0,  # Will be set later
            xo1t=0, xo2t=0, yo1t=0, yo2t=0,
            xi1r=xi1, xi2r=xi2, yi1r=yi1, yi2r=yi2,
            xo1r=xo1, xo2r=xo2, yo1r=yo1, yo2r=yo2
        )
    
    def _apply_overlap_adjustments(self, coords: TileCoordinates, 
                                   hindex: int, vindex: int) -> TileCoordinates:
    ...
    def _calculate_tile_internal_coords(self, coords: TileCoordinates,
                                       hindex: int, vindex: int) -> TileCoordinates:
...
def stitch_tiles(tiles: List[Dict[str, Any]], 
    ...
    for tile_data in tiles:
        ...
        # Place input
        inputs[coords.yi1r:coords.yi2r, coords.xi1r:coords.xi2r] = \
            tile_data['input'][coords.yi1t:coords.yi2t, coords.xi1t:coords.xi2t]
        
        # Place prediction
        preds[coords.yo1r:coords.yo2r, coords.xo1r:coords.xo2r] = \
            tile_data['prediction'][coords.yo1t:coords.yo2t, coords.xo1t:coords.xo2t]
        ...
        if has_ground_truth and groundtruths is not None:
            groundtruths[coords.yo1r:coords.yo2r, coords.xo1r:coords.xo2r] = \
                tile_data['ground_truth'][coords.yo1t:coords.yo2t, coords.xo1t:coords.xo2t]
...
def visualize_at_index(mm, dm, dm_vis, out_list, in_transform, out_transform,
                      variable, src, device, div, overlap, index=0, 
                      tensor_par_size=1, tensor_par_group=None,
    ...
    Args:
        ...
        div: Number of divisions for tiling
        overlap: Overlap between tiles
        index: Sample index to visualize
        tensor_par_size: Tensor parallelism size
        tensor_par_group: Tensor parallel process group
        config: Visualization configuration
    
    Returns:
        Animation object (currently None)
    """
    if config is None:
        config = VisualizationConfig()
    ...
    # Log information
    if dist.get_rank() == 0:
        logger.info(f"Input shape: {images['input'].shape}, "
                   f"range: [{images['input'].min():.4f}, {images['input'].max():.4f}]")
        logger.info(f"Prediction shape: {images['prediction'].shape}, "
